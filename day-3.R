...
doMove <- function(def, pos) {
  direction <- substr(def, 1, 1)
  amount <- as.numeric(substr(def, 2, 100000000))
  # message("Direction: ", direction, " amount: ", amount)
  instructions = moves[[direction]]
  result <- pos
  result[[instructions[[2]]]] <- result[[instructions[[2]]]] + amount * instructions[[1]]
  # message("Starting at ", paste(pos, collapse = ","), " got to ", paste(result, collapse = ","))
  list(result, amount)
}

doMove("R12", c(5,16))

strsplit("U7,R6,D4,L4", ",") %>% unlist()

getPoints <- function(path) {
  all.pos <- c(0,0)
  all.steps <- 0
  sapply(unlist(strsplit(path, ",")), function(def) {
    move.result <- doMove(def, tail(all.pos, 2))
    all.pos <<- c(all.pos, move.result[[1]])
    all.steps <<- c(all.steps, move.result[[2]])
  })
  list(
    all.pos %>% matrix(nrow = 2)
    , all.steps
  )
}

p <- getPoints("U7,R6,D4,L4")

apply(p, 2, function(x) x)

singleManhattan <- function(L1, L2) {
  i1 <- intersect(L1[1,1]:L1[1,2], L2[1,1]:L2[1,2])
  i2 <- intersect(L1[2,1]:L1[2,2], L2[2,1]:L2[2,2])
  if(!length(i1) || !length(i2))
    return(NULL)
  list(
    min(abs(i1)) + min(abs(i2))
    , i1
    , i2
  )
}

getIntersections <- function(path1, path2) {
  p1 <- getPoints(path1)
  p2 <- getPoints(path2)
  rv <- lapply(1:(dim(p1[[1]])[2] - 1), function(x1){
    L1 <- p1[[1]][, x1:(x1 + 1)]
    lapply(1:(dim(p2[[1]])[2] - 1), function(x2){
      L2 <- p2[[1]][, x2:(x2 + 1)]
      rmh <- singleManhattan(L1, L2)
      if(is.null(rmh))
        return(NULL)
      steps1 <- sum(p1[[2]][1:x1]) + min(abs(L1[1,1] - rmh[[2]])) + min(abs(L1[2,1] - rmh[[3]]))
      steps2 <- sum(p2[[2]][1:x2]) + min(abs(L2[1,1] - rmh[[2]])) + min(abs(L2[2,1] - rmh[[3]]))
      message("L1 = ", paste(L1, collapse = ","), "; L2 = ", paste(L2, collapse = ","), "; Mh = ", rmh[[1]], "; steps1 = ", steps1, "; steps2 = ", steps2)
      list(manhattan = rmh[[1]], steps = steps1 + steps2)
    })
  })# %>% bind_rows()
  rv %>% unlist(recursive = FALSE) %>% .[-which(sapply(., is.null))] %>% bind_rows()
  # rv %>% matrix(nrow = 2)
}

getIntersections(path1 = "R8,U5,L5,D3", path2 = "U7,R6,D4,L4")
getIntersections("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83")
getIntersections("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7")
puzzle.mh <- getIntersections(
...
puzzle.mh %>% arrange(manhattan)
puzzle.mh %>% arrange(steps)
